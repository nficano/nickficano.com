...
class HTMLCompress(Extension):
    ...
    block_elements = set([
        'div', 'p', 'form', 'ul', 'ol', 'li', 'table', 'tr', 'tbody', 'thead',
        'tfoot', 'tr', 'td', 'th', 'dl', 'dt', 'dd', 'blockquote', 'h1', 'h2',
        'h3', 'h4', 'h5', 'h6', 'pre',
    ])
    ...
    def leave_tag(self, tag, ctx):
        if not ctx.stack:
            ctx.fail(f'Tried to leave "{tag}" but something closed it already')
        if tag == ctx.stack[-1]:
            ctx.stack.pop()
            return
    ...
    def filter_stream(self, stream):
        ...
        while True:
            if stream.current.type == 'block_begin':
                if (
                    stream.look().test('name:strip') or
                    ...
                    if stream.current.type != 'block_end':
                        ctx.fail(
                            f'expected end of block, got '
                            f'{describe_token(stream.current)}',
                        )
                    stream.skip()
