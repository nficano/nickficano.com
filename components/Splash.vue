<template>
  <div class="fixed top-0 left-0 z-50 pointer-events-none">
    <canvas ref="canvas" id="fluid" class="w-screen h-screen" />
  </div>
</template>
...
function initFramebuffers() {
  let simRes = getResolution(props.SIM_RESOLUTION);
  let dyeRes = getResolution(props.DYE_RESOLUTION);
...
function updateColors(dt) {
  colorUpdateTimer += dt * props.COLOR_UPDATE_SPEED;
...
function step(dt) {
  ...
  for (let i = 0; i < props.PRESSURE_ITERATIONS; i++) {
...
function splatPointer(pointer) {
  let dx = pointer.deltaX * props.SPLAT_FORCE;
  let dy = pointer.deltaY * props.SPLAT_FORCE;
...
function splat(x, y, dx, dy, color) {
  ...
  gl.uniform1f(
    ...
    correctRadius(props.SPLAT_RADIUS / 100.0)
  );
  blit(velocity.write);
  velocity.swap();
